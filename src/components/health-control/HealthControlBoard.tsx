...
import { HealthFocusPickerSheet } from "@/components/health-control/HealthFocusPickerSheet";
import { createHealthTileViewModel, type HealthTileViewModel } from "@/components/health-control/healthTileTypes";
...
type ResolvedHealthFocusTile = HealthTileViewModel & {
  contextLine: string;
  displayTitle: string;
...
function resolveFocusTile(slot: HealthWidgetSlot, context: HealthFocusDataContext): ResolvedHealthFocusTile {
  ...
  const viewModel = createHealthTileViewModel({ displayData, focus: definition, slotId: slot.slotId });

  return {
    ...viewModel,
    contextLine: viewModel.copy.contextLine ?? "",
    definition,
    displayTitle: viewModel.copy.displayTitle,
    moduleLabel: viewModel.copy.moduleLabel,
    primaryValue: viewModel.copy.primaryValue,
  };
}
...
export function HealthRealmBoard({
  ...
  const [focusPickerSlotId, setFocusPickerSlotId] = useState<HealthWidgetSlot["slotId"] | null>(null);
  ...
  const pickerTile = useMemo(() => {
    const slot = widgetSlots.find((item) => item.slotId === focusPickerSlotId);

    return slot ? resolveFocusTile(slot, focusContext) : null;
  }, [focusContext, focusPickerSlotId, widgetSlots]);
  ...
  function handleTileAction(action: HealthTileMenuAction, tile: ResolvedHealthFocusTile): void {
    ...
    if (action === "replace") {
      setFocusPickerSlotId(tile.slotId);
    }
  }
  ...
  function replaceSlotFocus(slotId: HealthWidgetSlot["slotId"], focusId: string): void {
    // TODO BACKEND INTEGRATION: persist HealthBoardPreferences.
  ...
  return (
    <View style={[styles.container, framed ? styles.framedCard : styles.frameless, framed && raisedSurface(theme), { width: cardWidth }, style]}>
      ...
      {pickerTile ? (
        <HealthFocusPickerSheet
          currentFocus={{
            accentColor: pickerTile.definition.accentColor,
            currentValue: pickerTile.primaryValue,
            displayTitle: pickerTile.displayTitle,
            focusId: pickerTile.focusId,
            label: pickerTile.definition.label,
            slotLabel: pickerTile.slotId,
          }}
          isOpen={Boolean(focusPickerSlotId)}
          onClose={() => setFocusPickerSlotId(null)}
          onSelectFocus={(focusId) => {
            replaceSlotFocus(pickerTile.slotId, focusId);
            setFocusPickerSlotId(null);
          }}
        />
      ) : null}
...
function HealthRealmTileButton({
  ...
  return (
    <Pressable
          accessibilityLabel={tile.state === "private" ? `${tile.moduleLabel}. Private.` : tile.copy.accessibilitySummary}
        ...
        >
          ...
          <View style={[styles.moduleLabelZone, moduleLabelStyle]}>
            <Text adjustsFontSizeToFit ellipsizeMode="tail" minimumFontScale={0.86} numberOfLines={1} style={[styles.moduleLabelText, { color: textColor }]}>
              {tile.moduleLabel}
            </Text>
          </View>
            ...
            {isLeftColumn ? (
              <>
                <View style={styles.dataCopyLeft}>
                  <View style={styles.dataCopyStack}>
                    <Text adjustsFontSizeToFit ellipsizeMode="tail" minimumFontScale={0.86} numberOfLines={1} style={[styles.displayTitleText, { color: mutedTileText }]}>
                      {tile.displayTitle}
                    </Text>
                    <Text adjustsFontSizeToFit ellipsizeMode="tail" minimumFontScale={0.86} numberOfLines={1} style={[styles.contextLineText, { color: mutedTileText }]}>
                      {tile.contextLine}
                    </Text>
                  </View>
                </View>
                <View style={styles.primaryRight}>
                  <Text adjustsFontSizeToFit ellipsizeMode="tail" minimumFontScale={0.78} numberOfLines={1} style={[styles.primaryValueText, styles.textRight, { color: textColor }]}>
                    {tile.primaryValue}
                  </Text>
                </View>
              </>
              ...
              <>
                <View style={styles.primaryLeft}>
                  <Text adjustsFontSizeToFit ellipsizeMode="tail" minimumFontScale={0.78} numberOfLines={1} style={[styles.primaryValueText, { color: textColor }]}>
                    {tile.primaryValue}
                  </Text>
                </View>
                <View style={styles.dataCopyRight}>
                  <View style={styles.dataCopyStack}>
                    <Text adjustsFontSizeToFit ellipsizeMode="tail" minimumFontScale={0.86} numberOfLines={1} style={[styles.displayTitleText, styles.textRight, { color: mutedTileText }]}>
                      {tile.displayTitle}
                    </Text>
                    <Text adjustsFontSizeToFit ellipsizeMode="tail" minimumFontScale={0.86} numberOfLines={1} style={[styles.contextLineText, styles.textRight, { color: mutedTileText }]}>
                      {tile.contextLine}
                    </Text>
                  </View>
                </View>
...
function HealthWidgetFocusPopover({
  ...
  const headerSubtitle = `${tile.primaryValue} - ${tile.contextLine}`;
    ...
    <>
      ...
      <View style={[styles.focusPopover, raisedSurface(theme), positionStyle]}>
        ...
        <View style={styles.focusPopoverList}>
          <FocusPopoverRow description="Go to the selected tracker" label="Open focus" onPress={() => onAction("open", tile)} textColor={textColor} mutedColor={mutedColor} />
          <FocusPopoverRow description="Choose another focus source" label="Replace focus" onPress={() => onAction("replace", tile)} textColor={textColor} mutedColor={mutedColor} />
